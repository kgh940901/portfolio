...
(function(){
  ...
  var VIEW = '.fw-media, .shot-main, .hero-pf';   // 다른 페이지로 넘어가는 큰 썸네일
  ...
  var css = [
    ...
    '  transition:transform .3s cubic-bezier(.22,1,.36,1), opacity .2s ease, background-color .3s ease, border-color .3s ease; }',
    /* 기본 상태: 작은 흰 점 */
    ...
    /* .mcur span 이 display:block 을 갖고 있어 특이도를 올려야 flex 가 먹음 */
    '.mcur .mcur-txt{ width:46px; height:46px; margin:-23px 0 0 -23px; display:flex; align-items:center; justify-content:center;',
    '  font-size:10px; font-weight:800; letter-spacing:1.4px; color:#1a0e00; opacity:0; transform:scale(.6); }',
    '.mcur.link .mcur-dot{ transform:scale(.42); }',
    '.mcur.link .mcur-ring{ transform:scale(1); opacity:1; }',
    /* 썸네일: 주황 원 + VIEW */
    '.mcur.view .mcur-dot{ opacity:0; transform:scale(.2); }',
    '.mcur.view .mcur-ring{ transform:scale(1.7); opacity:1; background:#ff7a1a; border-color:#ff7a1a; }',
    '.mcur.view .mcur-txt{ opacity:1; transform:scale(1.7); }',
    /* 마우스를 누르는 동안 살짝 눌리는 느낌 */
  ...
  cur.innerHTML = '<span class="mcur-dot"></span><span class="mcur-ring"></span><span class="mcur-txt">VIEW</span>';
  ...
  addEventListener('pointerover', function(e){
    ...
    var isView = !!t.closest(VIEW);
    cur.classList.toggle('view', isView);
    cur.classList.toggle('link', !isView && !!t.closest(LINK));
  }, { passive:true });
